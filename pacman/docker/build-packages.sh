...
/usr/bin/git clone "https://github.com/ijacquez/libyaul-packages" || { panic "Unable to clone libyaul-packages repository" 1; }

cd libyaul-packages/pacman/linux || { panic "Directory path pacman/linux doesn't exist" 1; }
...
if /usr/bin/git diff --name-only PKGBUILD; then
    ...
    trap '/bin/rm -f '"${REPO_PATH}/yaul-git-${pkgver}"'-*.pkg.tar.zst' 1
