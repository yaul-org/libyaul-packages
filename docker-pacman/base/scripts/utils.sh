...
extract_pkgver() {
    ...
    [ -z "${_file}" ] && { panic "extract_pkgver: Invalid argument \$1" 1; }
...
extract_pkgrel() {
    ...
    [ -z "${_file}" ] && { panic "extract_pkgrel: Invalid argument \$1" 1; }
...
# Sync S3 bucket.
#
# $1 - Relative path to sync
sync_repo() {
    local _relpath="${1}"

    [ -z "${_relpath}" ] && { panic "sync_repo: Invalid argument \$1" 1; }

    /bin/bash "${BUILD_BASEPATH}/scripts/s3sync.sh" "${_relpath}" || exit 1
}
