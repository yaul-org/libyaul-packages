...
for os in "linux" "mingw-w64"; do
    docker run -it --rm --privileged -e REPO_OS="${os}" -e REPO_BRANCH="${REPO_BRANCH}" -e REPO_BASEPATH="/home/builder" --mount type=bind,source="${PWD}/base",target=/home/builder ijacquez/yaul-packages:latest ./scripts/package-build-yaul-examples.sh
done
